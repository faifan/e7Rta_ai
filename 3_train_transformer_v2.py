"""
第七史诗选秀辅助 - 4. 训练 Transformer 模型
"""

import os
os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'

import json
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
import sys
import random
from datetime import datetime

sys.stdout.reconfigure(encoding='utf-8')
from model import DraftTransformer

# ==================== 配置 ====================
DATA_FILE = 'output/all_clean_v2.json'
OUTPUT_MODEL = 'draft_transformer.pth'
HERO_LIST_FILE = 'hero_list_146.json'
CACHE_FILE = 'output/processed_data_cache_146.pth'

BATCH_SIZE = 128
EPOCHS = 50
LEARNING_RATE = 0.0005

D_MODEL = 192
NHEAD = 6
NUM_LAYERS = 3
DROPOUT = 0.2
MAX_SEQ_LEN = 20

WEIGHT_DECAY = 0.01
GRADIENT_CLIP = 1.0
LABEL_SMOOTHING = 0.1
FINALBAN_WEIGHT = 3.0  # finalban 样本较少但重要，上采样权重
WIN_LOSS_WEIGHT = 0.0  # 胜率预测辅助 loss 权重（已关闭，单局胜负噪声大）
WIN_PICK_WEIGHT  = 1.5  # 赢局 pick/ban 样本权重（赢家的选择更值得学）
LOSS_PICK_WEIGHT = 0.7  # 输局 pick/ban 样本权重

GRADIENT_ACCUM_STEPS = 1
EARLY_STOP_PATIENCE = 10
WARMUP_RATIO = 0.2


# ==================== 数据解析 ====================
def parse_deck(deck_data):
    if not deck_data:
        return [], []

    hero_list = []
    preban_list = []

    if isinstance(deck_data.get('preban_list'), list):
        preban_list = deck_data['preban_list']

    if isinstance(deck_data.get('hero_list'), list):
        for hero in deck_data['hero_list']:
            code = hero.get('hero_code')
            if code:
                hero_list.append({
                    'code': code,
                    'first_pick': hero.get('first_pick', 0),
                    'mvp': hero.get('mvp', 0),
                    'ban': hero.get('ban', 0)
                })

    return hero_list, preban_list


# ==================== 早停机制 ====================
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0.001):
        self.patience = patience
        self.min_delta = min_delta
        self.counter = 0
        self.best_loss = None
        self.early_stop = False
        self.best_val_acc = 0

    def __call__(self, val_loss, val_acc):
        if self.best_loss is None:
            self.best_loss = val_loss
            self.best_val_acc = val_acc
        elif val_loss > self.best_loss - self.min_delta:
            self.counter += 1
            print(f"  早停计数：{self.counter}/{self.patience}")
            if self.counter >= self.patience:
                print(f"  早停触发：验证损失连续{self.patience}轮未改善")
                self.early_stop = True
        else:
            self.best_loss = val_loss
            self.best_val_acc = max(val_acc, self.best_val_acc)
            self.counter = 0

        return val_acc > self.best_val_acc - self.min_delta


# ==================== 数据集（修复版） ====================
class DraftDataset(Dataset):
    def __init__(self, battles, hero_to_idx):
        self.samples = []
        self.hero_to_idx = hero_to_idx
        self.num_heroes = len(hero_to_idx)

        print("处理战斗数据...")
        for i, battle in enumerate(battles):
            self._process_battle(battle)
            if (i + 1) % 10000 == 0:
                print(f"  已处理 {i+1:,} / {len(battles):,} 场...")

        print(f"生成 {len(self.samples):,} 个训练样本")
        
        # 统计标签分布
        win_samples = sum(1 for s in self.samples if s['win'] == 1)
        print(f"胜率标签分布：胜 {win_samples:,} ({win_samples/len(self.samples)*100:.1f}%) / 负 {len(self.samples)-win_samples:,}")

    def _process_battle(self, battle):
        my_heroes_data, my_preban = parse_deck(battle.get('my_deck', {}))
        enemy_heroes_data, enemy_preban = parse_deck(battle.get('enemy_deck', {}))

        iswin_raw = battle.get('iswin', 0)
        is_win = iswin_raw == 1  # 只有1才是赢，2是输

        my_heroes = [h['code'] for h in my_heroes_data if h['code'] in self.hero_to_idx]
        enemy_heroes = [h['code'] for h in enemy_heroes_data if h['code'] in self.hero_to_idx]

        my_preban = [h for h in my_preban if h in self.hero_to_idx]
        enemy_preban = [h for h in enemy_preban if h in self.hero_to_idx]

        if len(my_heroes) != 5 or len(enemy_heroes) != 5:
            return

        my_first_pick = any(h.get('first_pick', 0) == 1 for h in my_heroes_data)
        my_finalban = next((h['code'] for h in my_heroes_data if h.get('ban') == 1), None)
        enemy_finalban = next((h['code'] for h in enemy_heroes_data if h.get('ban') == 1), None)

        # 原始视角
        self._generate_samples(my_heroes, enemy_heroes, my_preban, enemy_preban,
                               is_win, my_first_pick, my_finalban, enemy_finalban)
        # 互换视角数据增强（双倍数据）
        self._generate_samples(enemy_heroes, my_heroes, enemy_preban, my_preban,
                               not is_win, not my_first_pick, enemy_finalban, my_finalban)

    def _generate_samples(self, my_heroes, enemy_heroes, my_preban, enemy_preban,
                          is_win, my_first_pick, my_finalban, enemy_finalban):
        hero_seq = []
        side_seq = []
        phase_seq = []
        my_fp = 1 if my_first_pick else 2   # 我方视角的先后手ID
        enemy_fp = 2 if my_first_pick else 1  # 敌方视角的先后手ID

        # Preban 阶段（跳过空上文样本，空序列进 Transformer 全 masked 输出无意义）
        my_ban_seq = []
        my_ban_side = []
        my_ban_phase = []
        for i in range(len(my_preban)):
            if len(my_ban_seq) > 0:
                self.samples.append({
                    'hero_seq': my_ban_seq.copy(),
                    'side_seq': my_ban_side.copy(),
                    'phase_seq': my_ban_phase.copy(),
                    'target': self.hero_to_idx[my_preban[i]],
                    'phase': 0,
                    'win': 1 if is_win else 0,
                    'side': 1,
                    'first_pick': my_fp,
                    'weight': WIN_PICK_WEIGHT if is_win else LOSS_PICK_WEIGHT
                })
            my_ban_seq.append(self.hero_to_idx[my_preban[i]])
            my_ban_side.append(3)
            my_ban_phase.append(0)

        enemy_ban_seq = []
        enemy_ban_side = []
        enemy_ban_phase = []
        for i in range(len(enemy_preban)):
            if len(enemy_ban_seq) > 0:
                self.samples.append({
                    'hero_seq': enemy_ban_seq.copy(),
                    'side_seq': enemy_ban_side.copy(),
                    'phase_seq': enemy_ban_phase.copy(),
                    'target': self.hero_to_idx[enemy_preban[i]],
                    'phase': 0,
                    'win': 0 if is_win else 1,
                    'side': 2,
                    'first_pick': enemy_fp,
                    'weight': WIN_PICK_WEIGHT if not is_win else LOSS_PICK_WEIGHT
                })
            enemy_ban_seq.append(self.hero_to_idx[enemy_preban[i]])
            enemy_ban_side.append(3)
            enemy_ban_phase.append(0)

        for h in my_preban + enemy_preban:
            hero_seq.append(self.hero_to_idx[h])
            side_seq.append(3)
            phase_seq.append(0)

        if my_first_pick:
            picks_order = [
                ('my', 1), ('enemy', 2), ('my', 2),
                ('enemy', 2), ('my', 2), ('enemy', 1),
            ]
        else:
            picks_order = [
                ('enemy', 1), ('my', 2), ('enemy', 2),
                ('my', 2), ('enemy', 2), ('my', 1),
            ]

        my_idx = enemy_idx = current_phase = 0

        for side, count in picks_order:
            for _ in range(count):
                if side == 'my' and my_idx < len(my_heroes):
                    if len(hero_seq) > 0:
                        self.samples.append({
                            'hero_seq': hero_seq.copy(),
                            'side_seq': side_seq.copy(),
                            'phase_seq': phase_seq.copy(),
                            'target': self.hero_to_idx[my_heroes[my_idx]],
                            'phase': current_phase,
                            'win': 1 if is_win else 0,
                            'side': 1,
                            'first_pick': my_fp,
                            'weight': WIN_PICK_WEIGHT if is_win else LOSS_PICK_WEIGHT
                        })
                    hero_seq.append(self.hero_to_idx[my_heroes[my_idx]])
                    side_seq.append(1)
                    phase_seq.append(current_phase + 1)
                    my_idx += 1

                elif side == 'enemy' and enemy_idx < len(enemy_heroes):
                    if len(hero_seq) > 0:
                        self.samples.append({
                            'hero_seq': hero_seq.copy(),
                            'side_seq': side_seq.copy(),
                            'phase_seq': phase_seq.copy(),
                            'target': self.hero_to_idx[enemy_heroes[enemy_idx]],
                            'phase': current_phase,
                            'win': 0 if is_win else 1,
                            'side': 2,
                            'first_pick': enemy_fp,
                            'weight': WIN_PICK_WEIGHT if not is_win else LOSS_PICK_WEIGHT
                        })
                    hero_seq.append(self.hero_to_idx[enemy_heroes[enemy_idx]])
                    side_seq.append(2)
                    phase_seq.append(current_phase + 1)
                    enemy_idx += 1

            if current_phase < 5:
                current_phase += 1

        # Finalban 阶段
        # 规则：双方各自第3个选的英雄（index=2）不可被ban
        # 我方 Finalban：从敌方英雄中选1个禁用（排除enemy第3选）
        if enemy_finalban and enemy_finalban in self.hero_to_idx:
            enemy_pick_indices = [self.hero_to_idx[h] for i, h in enumerate(enemy_heroes)
                                  if h in self.hero_to_idx and i != 2]
            self.samples.append({
                'hero_seq': hero_seq.copy(),
                'side_seq': side_seq.copy(),
                'phase_seq': phase_seq.copy(),
                'target': self.hero_to_idx[enemy_finalban],
                'phase': 6,
                'win': 1 if is_win else 0,
                'side': 1,
                'first_pick': my_fp,
                'weight': FINALBAN_WEIGHT,
                'avail_indices': enemy_pick_indices
            })

        # 敌方 Finalban：从我方英雄中选1个禁用（排除my第3选）
        if my_finalban and my_finalban in self.hero_to_idx:
            my_pick_indices = [self.hero_to_idx[h] for i, h in enumerate(my_heroes)
                               if h in self.hero_to_idx and i != 2]
            self.samples.append({
                'hero_seq': hero_seq.copy(),
                'side_seq': side_seq.copy(),
                'phase_seq': phase_seq.copy(),
                'target': self.hero_to_idx[my_finalban],
                'phase': 6,
                'win': 0 if is_win else 1,
                'side': 2,
                'first_pick': enemy_fp,
                'weight': FINALBAN_WEIGHT,
                'avail_indices': my_pick_indices
            })

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        sample = self.samples[idx]
        avail_indices = sample.get('avail_indices')
        if avail_indices is not None:
            avail_mask = torch.zeros(self.num_heroes, dtype=torch.float)
            avail_mask[avail_indices] = 1.0
        else:
            avail_mask = torch.ones(self.num_heroes, dtype=torch.float)
        return {
            'hero_seq': torch.tensor(sample['hero_seq'], dtype=torch.long),
            'side_seq': torch.tensor(sample['side_seq'], dtype=torch.long),
            'phase_seq': torch.tensor(sample.get('phase_seq', [0] * len(sample['hero_seq'])), dtype=torch.long),
            'target': torch.tensor(sample['target'], dtype=torch.long),
            'phase': torch.tensor(sample['phase'], dtype=torch.long),
            'win': torch.tensor(sample['win'], dtype=torch.float),
            'side': torch.tensor(sample['side'], dtype=torch.long),
            'first_pick': torch.tensor(sample.get('first_pick', 1), dtype=torch.long),
            'weight': torch.tensor(sample['weight'], dtype=torch.float),
            'avail_mask': avail_mask
        }


def collate_fn(batch):
    max_len = max(len(item['hero_seq']) for item in batch)

    hero_seqs = torch.zeros(len(batch), max_len, dtype=torch.long)
    side_seqs = torch.zeros(len(batch), max_len, dtype=torch.long)
    phase_seqs = torch.zeros(len(batch), max_len, dtype=torch.long)
    targets = torch.zeros(len(batch), dtype=torch.long)
    phases = torch.zeros(len(batch), dtype=torch.long)
    wins = torch.zeros(len(batch), dtype=torch.float)
    sides = torch.zeros(len(batch), dtype=torch.long)
    first_picks = torch.zeros(len(batch), dtype=torch.long)
    masks = torch.zeros(len(batch), max_len, dtype=torch.float)
    weights = torch.zeros(len(batch), dtype=torch.float)
    avail_masks = torch.stack([item['avail_mask'] for item in batch])

    for i, item in enumerate(batch):
        seq_len = len(item['hero_seq'])
        hero_seqs[i, :seq_len] = item['hero_seq']
        side_seqs[i, :seq_len] = item['side_seq']
        phase_seqs[i, :seq_len] = item['phase_seq']
        targets[i] = item['target']
        phases[i] = item['phase']
        wins[i] = item['win']
        sides[i] = item['side']
        first_picks[i] = item['first_pick']
        masks[i, :seq_len] = 1
        weights[i] = item['weight']

    return {
        'hero_seqs': hero_seqs,
        'side_seqs': side_seqs,
        'phase_seqs': phase_seqs,
        'targets': targets,
        'phases': phases,
        'wins': wins,
        'sides': sides,
        'first_picks': first_picks,
        'masks': masks,
        'weights': weights,
        'avail_masks': avail_masks
    }


# ==================== 训练函数 ====================
def train_epoch(model, dataloader, optimizer, scheduler, criterion, criterion_no_smooth, win_criterion, device, epoch, accum_steps=1):
    model.train()
    total_loss = 0
    correct = 0
    top3_correct = 0
    top5_correct = 0
    total = 0

    from tqdm import tqdm
    pbar = tqdm(dataloader, desc=f"Epoch {epoch}", unit="batch")
    optimizer.zero_grad()

    for batch_idx, batch in enumerate(pbar):
        hero_seqs = batch['hero_seqs'].to(device)
        side_seqs = batch['side_seqs'].to(device)
        phase_seqs = batch['phase_seqs'].to(device)
        targets = batch['targets'].to(device)
        phases = batch['phases'].to(device)
        masks = batch['masks'].to(device)
        avail_masks = batch['avail_masks'].to(device)
        wins = batch['wins'].to(device)
        sides = batch['sides'].to(device)
        first_picks = batch['first_picks'].to(device)

        logits, win_rate = model(hero_seqs, side_seqs, phases, src_mask=masks, token_phase_ids=phase_seqs, prediction_side_ids=sides, first_pick_ids=first_picks)
        logits_masked = logits.masked_fill(avail_masks == 0, float('-inf'))

        is_finalban = (phases == 6)
        if is_finalban.any():
            losses = torch.where(
                is_finalban,
                criterion_no_smooth(logits_masked, targets),
                criterion(logits, targets)
            )
        else:
            losses = criterion(logits, targets)  # [batch]，reduction='none'

        pick_loss = (losses * batch['weights'].to(device)).mean()
        win_loss = win_criterion(win_rate.squeeze(-1), wins)
        total_loss_batch = (pick_loss + WIN_LOSS_WEIGHT * win_loss) / accum_steps
        total_loss_batch.backward()

        if (batch_idx + 1) % accum_steps == 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), GRADIENT_CLIP)
            optimizer.step()
            optimizer.zero_grad()

        total_loss += total_loss_batch.item() * accum_steps

        pred = logits.argmax(dim=-1)
        correct += (pred == targets).sum().item()
        top3_correct += (logits.topk(3, dim=-1).indices == targets.unsqueeze(1)).any(dim=1).sum().item()
        top5_correct += (logits.topk(5, dim=-1).indices == targets.unsqueeze(1)).any(dim=1).sum().item()
        total += targets.size(0)

        pbar.set_postfix({
            'loss': f'{total_loss/(batch_idx+1):.4f}',
            'acc': f'{correct/total:.4f}',
            'top3': f'{top3_correct/total:.4f}'
        })

        if scheduler is not None and isinstance(scheduler, optim.lr_scheduler.OneCycleLR):
            scheduler.step()

    return total_loss / len(dataloader), correct / total, top3_correct / total, top5_correct / total


def evaluate(model, dataloader, criterion, criterion_no_smooth, win_criterion, device):
    model.eval()
    total_loss = 0
    correct = 0
    top3_correct = 0
    top5_correct = 0
    total = 0

    from tqdm import tqdm
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="验证", unit="batch"):
            hero_seqs = batch['hero_seqs'].to(device)
            side_seqs = batch['side_seqs'].to(device)
            phase_seqs = batch['phase_seqs'].to(device)
            targets = batch['targets'].to(device)
            phases = batch['phases'].to(device)
            masks = batch['masks'].to(device)
            avail_masks = batch['avail_masks'].to(device)
            wins = batch['wins'].to(device)
            sides = batch['sides'].to(device)
            first_picks = batch['first_picks'].to(device)

            logits, win_rate = model(hero_seqs, side_seqs, phases, src_mask=masks, token_phase_ids=phase_seqs, prediction_side_ids=sides, first_pick_ids=first_picks)
            logits_masked = logits.masked_fill(avail_masks == 0, float('-inf'))

            is_finalban = (phases == 6)
            if is_finalban.any():
                losses = torch.where(
                    is_finalban,
                    criterion_no_smooth(logits_masked, targets),
                    criterion(logits, targets)
                )
                pick_loss = losses.mean()
            else:
                pick_loss = criterion(logits, targets).mean()

            win_loss = win_criterion(win_rate.squeeze(-1), wins)
            total_loss_batch = pick_loss + WIN_LOSS_WEIGHT * win_loss

            total_loss += total_loss_batch.item()

            pred = logits.argmax(dim=-1)
            correct += (pred == targets).sum().item()
            top3_correct += (logits.topk(3, dim=-1).indices == targets.unsqueeze(1)).any(dim=1).sum().item()
            top5_correct += (logits.topk(5, dim=-1).indices == targets.unsqueeze(1)).any(dim=1).sum().item()
            total += targets.size(0)

    return total_loss / len(dataloader), correct / total, top3_correct / total, top5_correct / total


# ==================== 主程序 ====================
if __name__ == '__main__':
    print("="*70)
    print("第七史诗 - 4. 训练 Transformer 模型（修复数据问题版）")
    print("="*70)

    if not os.path.exists(DATA_FILE):
        print(f"\n❌ 找不到数据文件 {DATA_FILE}")
        sys.exit(1)

    # 加载数据
    if os.path.exists(CACHE_FILE):
        print(f"\n发现缓存数据：{CACHE_FILE}")
        use_cache = input("是否使用缓存数据？(y/n): ").strip().lower()
        if use_cache == 'y':
            cache_data = torch.load(CACHE_FILE)
            hero_list = cache_data['hero_list']
            hero_to_idx = cache_data['hero_to_idx']
            idx_to_hero = cache_data['idx_to_hero']
            battles = cache_data['battles']
            print(f"✅ 已加载缓存数据：{len(battles):,} 场战斗，{len(hero_list)} 个英雄")

    if 'battles' not in locals():
        print(f"\n加载数据：{DATA_FILE}")
        with open(DATA_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)

        all_heroes = set()
        battles = []

        print("收集英雄代码...")
        for battle in data:
            battles.append(battle)

            my_heroes_data, my_preban = parse_deck(battle.get('my_deck', {}))
            enemy_heroes_data, enemy_preban = parse_deck(battle.get('enemy_deck', {}))

            for h in my_heroes_data + enemy_heroes_data:
                code = h.get('code')
                if code:
                    all_heroes.add(code)

        hero_list = sorted(list(all_heroes))
        hero_to_idx = {h: i for i, h in enumerate(hero_list)}
        idx_to_hero = {i: h for i, h in enumerate(hero_list)}

        print(f"\n保存数据缓存：{CACHE_FILE}")
        os.makedirs(os.path.dirname(CACHE_FILE), exist_ok=True)
        torch.save({
            'hero_list': hero_list,
            'hero_to_idx': hero_to_idx,
            'idx_to_hero': idx_to_hero,
            'battles': battles
        }, CACHE_FILE)

    print(f"英雄总数：{len(hero_list)}")
    print(f"战斗总数：{len(battles):,}")

    with open(HERO_LIST_FILE, 'w', encoding='utf-8') as f:
        json.dump({'hero_list': hero_list, 'hero_to_idx': hero_to_idx}, f, ensure_ascii=False, indent=2)

    # 去重：同一场对局从双方视角各记录一次，用 battle_seq 精确去重
    print("\n对局去重...")
    seen_seqs = set()
    deduped = []
    for battle in battles:
        key = battle.get('battle_seq')
        if key is None or key not in seen_seqs:
            if key is not None:
                seen_seqs.add(key)
            deduped.append(battle)
    print(f"去重前：{len(battles):,}  去重后：{len(deduped):,}  删除：{len(battles)-len(deduped):,}")
    battles = deduped

    random.shuffle(battles)
    split_idx = int(len(battles) * 0.9)
    train_battles = battles[:split_idx]
    val_battles = battles[split_idx:]

    print(f"\n训练集：{len(train_battles):,} 场")
    print(f"验证集：{len(val_battles):,} 场")

    print("\n创建训练数据集...")
    train_dataset = DraftDataset(train_battles, hero_to_idx)

    print("创建验证数据集...")
    val_dataset = DraftDataset(val_battles, hero_to_idx)

    train_loader = DataLoader(
        train_dataset, batch_size=BATCH_SIZE, shuffle=True,
        collate_fn=collate_fn, num_workers=0, pin_memory=True
    )
    val_loader = DataLoader(
        val_dataset, batch_size=BATCH_SIZE, shuffle=False,
        collate_fn=collate_fn, num_workers=0
    )

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n使用设备：{device}")

    model = DraftTransformer(
        num_heroes=len(hero_list),
        d_model=D_MODEL,
        nhead=NHEAD,
        num_layers=NUM_LAYERS,
        dropout=DROPOUT,
        max_seq_len=MAX_SEQ_LEN,
        dim_feedforward=D_MODEL * 4  # ⭐ FFN 维度应该是 d_model 的 4 倍
    ).to(device)

    print(f"模型参数量：{sum(p.numel() for p in model.parameters()):,}")

    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=LEARNING_RATE * 2,
        epochs=EPOCHS,
        steps_per_epoch=len(train_loader),
        pct_start=WARMUP_RATIO,
        anneal_strategy='cos',
        cycle_momentum=False
    )

    criterion = nn.CrossEntropyLoss(label_smoothing=LABEL_SMOOTHING, reduction='none')
    criterion_no_smooth = nn.CrossEntropyLoss(reduction='none')
    win_criterion = nn.BCELoss()
    early_stopping = EarlyStopping(patience=EARLY_STOP_PATIENCE)

    print("\n" + "="*70)
    print("开始训练...")
    print("="*70)
    print(f"配置参数:")
    print(f"  BATCH_SIZE={BATCH_SIZE}, EPOCHS={EPOCHS}, LEARNING_RATE={LEARNING_RATE}")
    print(f"  D_MODEL={D_MODEL}, NHEAD={NHEAD}, NUM_LAYERS={NUM_LAYERS}")
    print(f"  DROPOUT={DROPOUT}, WEIGHT_DECAY={WEIGHT_DECAY}, GRADIENT_CLIP={GRADIENT_CLIP}")
    print(f"  LABEL_SMOOTHING={LABEL_SMOOTHING}, FINALBAN_WEIGHT={FINALBAN_WEIGHT}")
    print(f"  DIM_FEEDFORWARD={D_MODEL * 4}")
    print(f"  早停：{EARLY_STOP_PATIENCE} 轮无提升则停止")
    print("="*70)

    best_val_loss = float('inf')
    start_time = datetime.now()

    for epoch in range(1, EPOCHS + 1):
        print(f"\nEpoch {epoch}/{EPOCHS}")
        print("-" * 50)

        train_loss, train_acc, train_top3, train_top5 = train_epoch(
            model, train_loader, optimizer, scheduler,
            criterion, criterion_no_smooth, win_criterion, device, epoch, accum_steps=GRADIENT_ACCUM_STEPS
        )
        val_loss, val_acc, val_top3, val_top5 = evaluate(model, val_loader, criterion, criterion_no_smooth, win_criterion, device)

        print(f"\n训练结果:")
        print(f"  训练损失：{train_loss:.4f}  Top1：{train_acc:.4f}  Top3：{train_top3:.4f}  Top5：{train_top5:.4f}")
        print(f"  验证损失：{val_loss:.4f}  Top1：{val_acc:.4f}  Top3：{val_top3:.4f}  Top5：{val_top5:.4f}")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save({
                'model_state_dict': model.state_dict(),
                'hero_to_idx': hero_to_idx,
                'idx_to_hero': idx_to_hero,
                'config': {
                    'd_model': D_MODEL,
                    'nhead': NHEAD,
                    'num_layers': NUM_LAYERS,
                    'dropout': DROPOUT,
                    'num_heroes': len(hero_list),
                    'dim_feedforward': D_MODEL * 4
                },
                'val_acc': val_acc,
                'epoch': epoch,
                'train_loss': train_loss,
                'val_loss': val_loss
            }, OUTPUT_MODEL)
            print(f"✅ 保存最佳模型 (验证损失：{val_loss:.4f}  Top1：{val_acc:.4f})")

        early_stopping(val_loss, val_acc)
        if early_stopping.early_stop:
            print(f"\n训练提前结束，最佳验证损失：{best_val_loss:.4f}")
            break

    elapsed = datetime.now() - start_time
    print("\n" + "="*70)
    print(f"训练完成！耗时：{elapsed}")
    print(f"最佳验证损失：{best_val_loss:.4f}")
    print(f"模型已保存：{OUTPUT_MODEL}")
    print("="*70)
